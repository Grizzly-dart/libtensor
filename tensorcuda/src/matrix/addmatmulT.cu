...
template <typename T>
__global__ void matmulTCadd(
    T *out, T *inp1, T* inp2T, T* add, uint32_t m, uint32_t n, uint32_t k
) {
  ...
  if (outRow < m && outCol < k) {
    out[outRow * k + outCol] = sum + add[outCol];
  }
}
...
char const *libtcCudaMatMulTCadd(
  ...
  err = cudaLaunchKernelEx(&config, matmulTCadd<double>, out, inp1, inp2T, add, m, n, k);
  if (err != cudaSuccess) {
    return cudaGetErrorString(err);
  }
  return nullptr;
